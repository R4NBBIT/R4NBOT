...
def _collect_participant_mentions(entry: dict) -> list[str]:
    """대기열은 제외하고, 실제 참가자(딜러+서포터)만 멘션 문자열로 반환. 용병은 멘션할 디스코드 계정이 없어서 제외됨."""
    mentions: list[str] = []
...
# =========================
class TagParticipantsModal(discord.ui.Modal):
    def __init__(self, cog: "RaidScheduleCog", raid_id: str):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        mentions = _collect_participant_mentions(entry)
        if not mentions:
...
class RaidPostView(discord.ui.View):
    ...
    @discord.ui.button(label="참가자 태그", style=discord.ButtonStyle.gray, custom_id="raid_tag_participants")
    async def tag_participants(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        is_participant = loc is not None and loc["where"] == "participant"
        if not (is_creator or is_admin or is_participant):
            await interaction.response.send_message(
        ...
        mentions = _collect_participant_mentions(entry)
        if not mentions:
            await interaction.response.send_message("태그할 참가자가 없어요. (대기열 인원은 태그 대상이 아니에요)", ephemeral=True)
            return

        modal = TagParticipantsModal(cog, raid_id)
        await interaction.response.send_modal(modal)
